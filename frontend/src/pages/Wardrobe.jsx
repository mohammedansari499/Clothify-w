...
export default function Wardrobe() {
  ...
  const handleResetWorn = async (id) => {
    try {
      setClothes(prev => prev.map(c =>
  ...
  clothes.forEach(item => {
    ...
    if (!groupedBySection[group]) groupedBySection[group] = {};
    if (!groupedBySection[group][type]) groupedBySection[group][type] = [];
    groupedBySection[group][type].push(item);
  });

  const totalItems = clothes.length;
  const totalCategories = new Set(clothes.map(c => c.type)).size;
  const totalGroups = Object.keys(groupedBySection).length;
  ...
  return (
    <div className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          {totalItems > 0 && (
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg">
                <span className="text-white font-bold text-lg">{totalCategories}</span>
                <span className="text-gray-400 text-xs">Types</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg">
                <span className="text-white font-bold text-lg">{totalGroups}</span>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 rounded-3xl border border-white/5 bg-white/[0.01] backdrop-blur-xl shadow-2xl relative overflow-hidden">
                    ...
                    <div key={type}>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {items.map(item => {
                          ...
                          return (
                            <div key={item._id} className="group relative bg-white/[0.02] backdrop-blur-xl rounded-2xl border border-white/5 overflow-hidden hover:border-primary/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_30px_-10px_rgba(0,255,178,0.15)] flex flex-col">
                              {/* Image */}
                              <div className="aspect-square bg-gradient-to-b from-white/5 to-transparent w-full overflow-hidden flex items-center justify-center p-3 relative">
                                <img src={item.image_url} alt={item.type} className="object-contain w-full h-full drop-shadow-2xl group-hover:scale-105 transition-transform duration-500" />
                                ...
                                <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                  ...
                                  {(item.wear_count || 0) > 0 && (
                                    <button
                                      onClick={() => handleResetWorn(item._id)}
                              ...
                              <div className="p-3 space-y-2 border-t border-white/5">
                                ...
                                {/* Style badge */}
                                {item.style && (
                                  <span className={`inline-block text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 rounded border ${styleClass}`}>
                                    {item.style}
                                  </span>
                                )}
                                ...
                                {item.occasion_tags && item.occasion_tags.length > 0 && (
                                  <div className="flex flex-wrap gap-1">
                                    {item.occasion_tags.slice(0, 3).map(tag => (
                                      <span key={tag} className="text-[9px] uppercase font-bold tracking-wider px-1.5 py-0.5 bg-white/5 text-gray-500 rounded">
                                        {tag}
                                      </span>
                                    ))}
                                  </div>
