  ...
  return (
    ...
    >
      ...
      <div className="p-5 space-y-4 relative z-10">
        ...
        <div className="flex items-center justify-between">
          <div
            className={`text-[10px] font-black uppercase tracking-[0.15em] px-3 py-1 rounded-lg border bg-gradient-to-br transition-all duration-500 ${styleClass}`}
          >
            {item.style}
          </div>
...
export default function Wardrobe() {
  ...
  const filteredClothes = useMemo(() => {
    return clothes.filter((item) => {
      const matchesSearch = (item.type + item.style + (item.color_name || ""))
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const config = CATEGORY_CONFIG[item.type] || CATEGORY_CONFIG.unknown;
      const matchesGroup =
        activeGroup === "All" || config.group === activeGroup;
  ...
  const stats = useMemo(
    () => ({
      ...
      groups: GROUP_ORDER.filter((g) =>
        clothes.some(
          (c) =>
            (CATEGORY_CONFIG[c.type] || CATEGORY_CONFIG.unknown).group === g,
        ),
      ),
    }),
    [clothes],
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start justify-between relative z-10">
          <div className="max-w-xl">
            ...
            <p className="text-lg text-text-muted font-medium mb-10 leading-relaxed">
              Explore your collection through our neural classification system.
              Every fiber, color, and silhouette indexed for perfect
              coordination.
            </p>
        ...
        {loading ? (
          ...
          >
            ...
            <p className="text-text-muted max-w-md mx-auto font-medium">
              Your search criteria didn't yield any results from our neural
              indexing.
            </p>
            <button
              onClick={() => {
              ...
              >
                ...
                <div className="flex items-end gap-6 mb-12">
                  ...
                  <div className="mb-2 px-4 py-1.5 bg-surface border border-border-subtle rounded-2xl text-[10px] font-black uppercase tracking-widest text-text-muted">
                    {Object.values(groupedBySection[groupName]).flat().length}{" "}
                    Units
                  </div>
                </div>
                ...
                <div className="space-y-16">
                  {Object.keys(groupedBySection[groupName]).map((type) => {
                    const config =
                      CATEGORY_CONFIG[type] || CATEGORY_CONFIG.unknown;
                      ...
                      <div key={type} className="relative">
                        <div className="flex items-center gap-3 mb-8">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] font-bold text-primary/50">
                              {items.length} Variations Indexed
                            </div>
                          </div>
                        </div>
