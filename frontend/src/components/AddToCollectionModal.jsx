...
const AddToCollectionModal = ({ isOpen, onClose, outfit, onAdd }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-xl">
            <div className="w-full max-w-md bg-surface border border-border-subtle rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in duration-300">
                ...
                <div className="space-y-4 mb-6">
                        ...
                        {collections.length === 0 && !isLoading ? (
                            ...
                            collections.map((coll) => (
                                <button
                                    ...
                                    className={`flex items-center justify-between p-3 rounded-xl border transition-all duration-200 ${selectedCollectionId === coll._id
                                        ? 'bg-secondary/20 border-secondary text-text shadow-[0_0_15px_rgba(14,165,233,0.3)]'
                                        : 'bg-card border-border-subtle text-text-muted hover:bg-card hover:border-border-subtle'
                                        }`}
                                >
                    ...
                    <form onSubmit={handleCreateCollection} className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-card border border-border-subtle rounded-xl px-4 py-2 text-sm text-text focus:outline-none focus:border-secondary transition-colors"
                        />
                        <button
                            type="submit"
                ...
                <div className="flex gap-3">
                    <button
                        onClick={onClose}
                        className="flex-1 px-4 py-2.5 rounded-xl border border-border-subtle text-text-muted hover:bg-primary/10 transition-colors font-medium"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        className="flex-1 px-4 py-2.5 bg-secondary hover:bg-secondary-dark rounded-xl text-text font-semibold shadow-lg shadow-secondary/20 transition-all active:scale-[0.98] disabled:opacity-50 disabled:active:scale-100 flex items-center justify-center gap-2"
                    >
                        {isLoading ? <Loader2 className="animate-spin text-text" size={18} /> : 'Save Outfit'}
                    </button>
                </div>
            </div>
        </div>
